...
Partial Class Creator
    ...
    Public Shared Function RunLiveQuery(sqlQuery As String, level As Integer) As JsonResponse
        ...
        Using conn As New SqlConnection(connectionString)
            Try
                ...
                Using cmd As New SqlCommand(sqlQuery, conn)
                    ...
                    Using adapter As New SqlDataAdapter(cmd)
                        ...
                        Dim levelData As New LiveSessionManager.LiveSessionLevel With {
                            .Level = level,
                            .SqlQuery = sqlQuery,
                            .Data = dataRows,
                            .Columns = columns
                        }
                        sessionManager.UpdateSession(levelData)
                        Dim hubContext = GlobalHost.ConnectionManager.GetHubContext(Of LivePreviewHub)()
                        hubContext.Clients.All.updateViewer(sessionManager)
                        response.Success = True
                        response.Data = New With {
                            .LocalData = dataRows,
                            .Columns = columns,
                            .CurrentLevel = sessionManager.CurrentVisibleLevel,
                            .MaxLevel = sessionManager.GetMaxLevel
                        }
                    End Using
                End Using
            Catch ex As Exception
                ...
                Dim hubContext = GlobalHost.ConnectionManager.GetHubContext(Of LivePreviewHub)()
                hubContext.Clients.All.updateViewer(Nothing) ' Send null to reset viewer on error
            End Try
        End Using
        Return response
    End Function
    ...
    Public Shared Function ResetLiveSession() As JsonResponse
        LiveSessionManager.Current.ResetSession()
        Dim hubContext = GlobalHost.ConnectionManager.GetHubContext(Of LivePreviewHub)()
        hubContext.Clients.All.resetViewer()
    ...
    Public Shared Function SaveReport(reportData As ReportSaveDTO) As JsonResponse
        ...
        If String.IsNullOrWhiteSpace(reportData.ReportName) OrElse String.IsNullOrWhiteSpace(reportData.SQLQuery) OrElse String.IsNullOrWhiteSpace(reportData.AllowedUsersCSV) Then
            response.Success = False : response.ErrorMessage = "Report Name, SQL Query, and Allowed Users cannot be empty."
            Return response
        End If
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Using conn As New SqlConnection(connectionString)
            ...
            Using tran As SqlTransaction = conn.BeginTransaction()
                Try
                    ...
                    If reportId = 0 Then
                        Dim insertSql As String = "INSERT INTO SavedReports (ReportName, Description, SQLQuery, CreatedBy, AllowedUsers, CreatedDate, LastModified) " & "VALUES (@ReportName, @Description, @SQLQuery, @CreatedBy, @AllowedUsers, GETDATE(), GETDATE());" & "SELECT SCOPE_IDENTITY();"
                        Using cmd As New SqlCommand(insertSql, conn, tran)
                            ...
                            cmd.Parameters.AddWithValue("@AllowedUsers", reportData.AllowedUsersCSV)
                            reportId = Convert.ToInt32(cmd.ExecuteScalar())
                        End Using
                    Else
                        Dim updateSql As String = "UPDATE SavedReports SET ReportName = @ReportName, Description = @Description, SQLQuery = @SQLQuery, AllowedUsers = @AllowedUsers, LastModified = GETDATE() WHERE ReportID = @ReportID;"
    ...
    <WebMethod()>
    Public Shared Function GetReportDetails(reportId As Integer) As JsonResponse
        Dim response As New JsonResponse()
        Dim connectionString As String = ConfigurationManager.ConnectionStrings("DefaultConnection").ConnectionString
        Using conn As New SqlConnection(connectionString)
            Try
                conn.Open()
                Dim sql As String = "SELECT ReportID, ReportName, Description, SQLQuery, AllowedUsers FROM SavedReports WHERE ReportID = @ReportID"
                Using cmd As New SqlCommand(sql, conn)
                    cmd.Parameters.AddWithValue("@ReportID", reportId)
                    Using reader As SqlDataReader = cmd.ExecuteReader()
                        If reader.Read() Then
                            response.Success = True
                            response.Data = New ReportDetailsDTO With {
                                .ReportID = Convert.ToInt32(reader("ReportID")),
                                .ReportName = reader("ReportName").ToString(),
                                .Description = reader("Description").ToString(),
                                .SQLQuery = reader("SQLQuery").ToString(),
                                .AllowedUsersCSV = reader("AllowedUsers").ToString()
                            }
                        Else
            ...
            Catch ex As Exception
                response.Success = False
                response.ErrorMessage = "Error fetching report details: " & ex.Message
            End Try
        End Using
        Return response
    End Function
